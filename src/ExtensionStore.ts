...
export class ExtensionStore {
	...
	constructor(globalState: vscode.ExtensionContext['globalState']) {
		...
		this.disabledExtensions = globalState.get<ExtensionData[]>(ExtensionStoreKey.DISABLED_EXTENSIONS);
	...
	async updateVSCodeVersion(): Promise<void> {
		await this.globalState.update(ExtensionStoreKey.VSCODE_VERSION, vscode.version);
	}
}
